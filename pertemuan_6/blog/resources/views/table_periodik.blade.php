...
<table id="examples" class="hide_optional">
    <tr>
        <td>&#xD7;</td>
        <td>&#xD7;</td>
        <td>&#xD7;</td>
...
function performSearch() {
        ...
        for (var i = 0; i < elements.length; i++) {
            var element = elements[i];
            var title = document.getElementById(element).getAttribute('title').toUpperCase();
            var id = element.toUpperCase();

            if (title.includes(searchInput) || id.includes(searchInput)) {
                // Highlight the matching element
                document.getElementById(element).classList.add('sel');
                state[i] = true;
            } else {
                // Remove highlight from non-matching elements
                document.getElementById(element).classList.remove('sel');
                state[i] = false;
            }
        }
